...
The existing pair-assessment evaluator now supports an optional, synthetic-only common-goal adaptation assessment with seven independent predicates and negative controls. See `tools/evaluate_common_goal_adaptation.py` and `tests/test_common_goal_adaptation.py`. This does not grant execution authority or substitute for Interlock/InTr and Master Records verification.

Post-revocation protection in draft PR #31: `selected_goal_after_revocation` is checked against an independently recomputed intersection of currently consenting, non-revoked participants (minimum two). A stale prior shared goal or missing post-revocation selection fails the synthetic agency predicate; no source-only assessment authorizes an actual transition.
